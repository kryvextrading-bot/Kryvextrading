...
-- 3. Update trading_settings with options profit rates
...
SET 
  metadata = jsonb_set(
    ...
    '{
      ...
      "360": 125,
      "600": 130,
      "last_aligned": "' || to_jsonb(now())::text || '"
    }'::jsonb
  ),
  updated_at = NOW()
...
-- 8. Update existing options transactions to have correct structure
...
SET 
  ...
  price = CASE 
    ...
    ELSE 
      0
    END
WHERE type = 'option' AND (pnl IS NULL OR price IS NULL);

-- 9. Create view for options trading statistics
...
-- 10. Grant select permissions on the view
