-- Safe Users Table Schema - Handles existing objects
-- This script creates/updates the users table without breaking existing data
...
CREATE TABLE IF NOT EXISTS public.users (
  ...
  -- Constraints (only add if they don't exist)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_kyc_status_check') THEN
        ALTER TABLE public.users ADD CONSTRAINT users_kyc_status_check CHECK (
            kyc_status = ANY (ARRAY['Verified'::text, 'Pending'::text, 'Rejected'::text])
        );
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_risk_tolerance_check') THEN
        ALTER TABLE public.users ADD CONSTRAINT users_risk_tolerance_check CHECK (
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'users_investment_goal_check') THEN
        ALTER TABLE public.users ADD CONSTRAINT users_investment_goal_check CHECK (
