-- Setup Trade Outcomes Table with LOSS by Default Logic
-- This script creates the necessary tables and sample data for the admin force win/loss system

-- First, ensure the trade_outcomes table exists with the correct structure
...
CREATE INDEX IF NOT EXISTS idx_trade_outcomes_enabled ON public.trade_outcomes(enabled, expires_at);
CREATE INDEX IF NOT EXISTS idx_trade_outcomes_priority ON public.trade_outcomes(priority, created_at);
...
DROP TRIGGER IF EXISTS update_trade_outcomes_updated_at ON public.trade_outcomes;
...
-- 4. Time-limited force WIN (expires after 24 hours)
INSERT INTO public.trade_outcomes (
    user_id,
    enabled,
    outcome_type,
...
BEGIN
    ...
    WHERE to2.user_id = p_user_id
        ...
        AND (to2.expires_at IS NULL OR to2.expires_at > NOW())
...
SELECT 
    ...
    u.last_name,
    CASE 
        WHEN to2.expires_at IS NULL THEN 'Never'
        ELSE to2.expires_at::text
    END as expires_in,
    CASE
...
-- Create a function to clean up expired outcomes (run via cron)
CREATE OR REPLACE FUNCTION cleanup_expired_trade_outcomes()
...
BEGIN
    WITH expired AS (
        ...
        SET enabled = false,
            updated_at = NOW(),
            description = COALESCE(description, '') || ' [AUTO-DISABLED: Expired at ' || NOW()::text || ']'
        WHERE enabled = true 
...
-- Grant execute permission
GRANT EXECUTE ON FUNCTION cleanup_expired_trade_outcomes TO service_role;

-- Test the cleanup function
SELECT cleanup_expired_trade_outcomes();
