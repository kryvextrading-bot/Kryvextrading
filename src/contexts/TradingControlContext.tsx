...
interface TradingControlContextType {
  userOutcome: TradeOutcome | null;
  activeWindows: TradeWindow[];
  systemSettings: SystemSettings | null;
  loading: boolean;
  countdown: string;
  shouldWin: (tradeType: string) => Promise<boolean>;
  getActiveWindow: () => TradeWindow | undefined;
  reload: () => void;
}

const TradingControlContext = createContext<TradingControlContextType | undefined>(undefined);

interface TradingControlProviderProps {
  children: ReactNode;
}

export function TradingControlProvider({ children }: TradingControlProviderProps) {
  const { user } = useAuth();
  ...
  const shouldWin = async (tradeType: string): Promise<boolean> => {
    if (!user?.id) return false;
    return tradingControlApi.checkTradeOutcome(user.id, tradeType);
  };

  // Get current active window
  const getActiveWindow = (): TradeWindow | undefined => {
    const now = new Date();
  ...
  const value: TradingControlContextType = {
    userOutcome,
    activeWindows,
    systemSettings,
    loading,
  ...
  return (
    <TradingControlContext.Provider value={value}>
      {children}
    </TradingControlContext.Provider>
  );
}

export function useTradingControl() {
  const context = useContext(TradingControlContext);
  if (context === undefined) {
    throw new Error('useTradingControl must be used within a TradingControlProvider');
  }
  return context;
}
