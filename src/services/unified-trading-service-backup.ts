...
class UnifiedTradingService {
  ...
  async expireOptionsTrade(tradeId: string): Promise<ExpireOptionsResult> {
    try {
      ...
      const payoutRate = Number(trade.metadata?.payoutRate) || 1.18; // Default to 18% profit for 60s
      const profit = wins ? premium * (payoutRate - 1) : -premium; // Net profit (positive for win, negative for loss)
      const totalReturn = wins ? premium * payoutRate : 0; // Full return: stake + profit
      ...
      const { error: updateError } = await supabaseAdmin
        ...
        .update({
          ...
          metadata: {
            ...
            profit: profit,
            exitPrice: trade.metadata?.entryPrice || trade.price // Use entry price as exit price for options
          },
      ...
      if (wins) {
        ...
        if (currentBalance) {
          ...
          const newLocked = Number(currentBalance.locked) - premium;
        ...
        // Loss: Release locked funds (they're already deducted, just update locked balance)
        const { data: currentBalance } = await supabase
          .from('wallet_balances')
          .select('locked')
          .eq('user_id', trade.user_id)
      ...
      // Also update the options_orders table if it exists
      try {
        await supabase
          .from('options_orders')
          .update({
            ...
            pnl: profit,
            expiry_price: trade.metadata?.entryPrice || trade.price
          })
          .eq('id', tradeId);
      } catch (optionsError) {
        console.log('Options orders table may not exist, continuing...');
      }
      ...
      return { 
        ...
        result, 
        profit,
        exitPrice: trade.metadata?.entryPrice || trade.price || 0
      };
      ...
      case 'options':
        trade = {
          ...
          metadata: {
            ...
            payoutRate: data.payoutRate || 1.18,
            entryPrice: data.entryPrice || data.price,
            startTime: now,
            endTime: new Date(Date.now() + (data.duration || 60) * 1000).toISOString()
          },
          createdAt: now,
          updatedAt: now
        } as OptionContract;
